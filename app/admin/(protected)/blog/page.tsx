  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: 26, color: "var(--ink)", margin: 0 }}>Blog Posts</h1>
        <Link
          href="/admin/blog/new"
      ...
      {!posts?.length ? (
        ...
        <div style={{ background: "white", borderRadius: 12, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {posts.map((post, i) => (
                <tr key={post.id} style={{ borderBottom: i < posts.length - 1 ? "1px solid rgba(0,0,0,0.06)" : "none" }}>
                  <td style={{ padding: "14px 20px" }}>
                    <Link href={`/admin/blog/${post.id}`} style={{ color: "var(--ink)", fontWeight: 600, textDecoration: "none", fontSize: 14 }}>
                      {post.title}
                    </Link>
                  </td>
                  <td style={{ padding: "14px 20px", fontFamily: "monospace", fontSize: 12, color: "rgba(0,0,0,0.4)" }}>
                    {post.slug}
                  </td>
                  <td style={{ padding: "14px 20px" }}>
                    <span style={{
